...
func TestSeparateRoman(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.input, func(t *testing.T) {
			...
			if operand1 != tc.expectedOperand1 || operand2 != tc.expectedOperand2 ||
				operator != tc.expectedOperator {
				t.Errorf(
					"Для входа '%s' ожидались операнды '%s', '%s' и оператор '%s'",
					tc.input,
					tc.expectedOperand1,
					tc.expectedOperand2,
					tc.expectedOperator,
